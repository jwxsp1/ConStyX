...
from networks.semantic_aug_ResUnet_scale import ResUnet
from config import *
import numpy as np
    ...
    def run(self):
        ...
        for epoch in tqdm(range(self.num_epochs)):
            ...
            for batch, data in enumerate(self.train_loader):
                ...
                first_pred= self.model(x,ori_mask,self.pix_grad,aug=False)
                loss = self.seg_cost(first_pred, y)
                self.optimizer.zero_grad()
                loss.backward(retain_graph=True) ###########
                end_time1 = time.time()
                start_time2 = time.time()
                pred = self.model(x,ori_mask,self.pix_grad.detach(),aug=True)
            ...
            if (epoch + 1) % self.valid_frequency == 0 and self.valid_loader is not None:
                ...
                if ((float(result_dict['Disc_Dice'])+float(result_dict['Cup_Dice']))/2>best_val_avg) or best_val_avg==0.0 :

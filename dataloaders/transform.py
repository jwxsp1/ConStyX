...
def collate_fn_w_transform(batch):
    image, label, name ,ori_mask= zip(*batch)
    image = np.stack(image, 0)
    label = np.stack(label, 0)
    name = np.stack(name, 0)
    ori_mask = np.stack(ori_mask, 0)
    data_dict = {'data': image, 'mask': label, 'name': name,'ori_mask':ori_mask}
    tr_transforms = get_train_transform()
    data_dict = tr_transforms(**data_dict)
    data_dict['mask'] = to_one_hot_list(data_dict['mask'])
    data_dict['ori_mask'] = data_dict['ori_mask']
    return data_dict


def collate_fn_wo_transform(batch):
    image, label, name,_= zip(*batch)
    image = np.stack(image, 0)
    label = np.stack(label, 0)
    name = np.stack(name, 0)

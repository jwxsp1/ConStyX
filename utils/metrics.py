...
def measure_img(o_img, t_num=1):
    ...
    testa1,num= measure.label(o_img.astype("bool"),return_num=True)
...
def data_process(pred, label, threshold=0.5):
    ...
    if not np.all(prediction_copy == 0):
        ...
        pred2 = prediction_u * prediction_copy
    else:
        pred2 =prediction_copy 

    pred=np.stack((pred1,pred2),1)
...
def asd_compute(test, reference):
    ...
    return np.mean(disc_asds), np.mean(cup_asds)
